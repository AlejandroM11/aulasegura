...
export default function Register(){
  ...
  const submit=e=>{
    ...
    list.push(u); localStorage.setItem('users',JSON.stringify(list))
    setUser(u); nav(role==='docente'?'/docente':'/estudiante')
  }
  return (
    <div className="card max-w-md mx-auto">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div><label className="label">Rol</label><select className="input" value={role} onChange={e=>setRole(e.target.value)}><option value="estudiante">Estudiante</option><option value="docente">Docente</option></select></div>
